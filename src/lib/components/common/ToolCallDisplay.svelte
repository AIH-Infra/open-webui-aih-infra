<script lang="ts">
	...
	const METADATA_KEEP = new Set([
		'page_start', 'page_end', 'print_page_start', 'print_page_end',
		'Header 1', 'Header 2', 'title'
	]);

	function cleanQueryResult(data: unknown): unknown {
		if (!Array.isArray(data)) return data;
		return data.map((chunk: any) => {
			const out: any = {};
			if (chunk.content !== undefined) out.content = chunk.content;
			if (chunk.source !== undefined) out.source = chunk.source;
			if (chunk.file_id !== undefined) out.file_id = chunk.file_id;
			if (chunk.distance !== undefined) out.distance = chunk.distance;
			if (chunk.metadata && typeof chunk.metadata === 'object') {
				const m: any = {};
				for (const k of METADATA_KEEP) {
					if (chunk.metadata[k] !== undefined) m[k] = chunk.metadata[k];
				}
	...
	$: parsedResult = (() => {
		const raw = parseJSONString(result);
		if (attributes?.name === 'query_knowledge_files' && Array.isArray(raw)) {
