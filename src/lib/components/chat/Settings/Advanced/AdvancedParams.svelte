<script lang="ts">
	...
	const getChangeSignature = (value: any) => {
		try {
	...
	onMount(() => {
		lastChangeSignature = getChangeSignature(params);
		changeReady = true;
	});

	$: if (params && changeReady) {
		const nextSignature = getChangeSignature(params);
...
<div class=" space-y-1 text-xs pb-safe-bottom">
	...
	<div>
		<Tooltip
			content={$i18n.t(
				"Managed mode uses Open WebUI's built-in tool orchestration for broader compatibility. Native mode leverages the model's built-in tool-calling capabilities, but requires the model to inherently support this feature."
			)}
			placement="top-start"
			className="inline-tooltip"
		>
			<div class=" py-0.5 flex w-full justify-between">
				...
				>
					...
					{:else}
						<span class="ml-2 self-center">{$i18n.t('Managed')}</span>
					{/if}
				</button>
			</div>
		</Tooltip>
