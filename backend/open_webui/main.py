...
async def chat_completion(
    ...
    try:
        ...
        # RAG Params (session-level parameters for knowledge base queries)
        rag_params = form_data.get("params", {}).get("rag", {})
        log.info(f"[RAG DEBUG MAIN] Received params from request: {form_data.get('params', {})}")
        log.info(f"[RAG DEBUG MAIN] Extracted rag_params: {rag_params}")
        ...
        metadata = {
            ...
            "params": {
                ...
                "rag": rag_params,  # Session-level RAG parameters
            },
        }

        rag_debug_info = metadata.get('params', {}).get('rag', {})
        log.info(f"[RAG DEBUG MAIN] Final metadata.params.rag: {rag_debug_info}")
