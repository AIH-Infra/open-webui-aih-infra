...
from open_webui.utils.rag import normalize_rag_mode_value
...
app.state.config.RAG_RERANKING_PRESET_MODELS = RAG_RERANKING_PRESET_MODELS
...
if app.state.config.RAG_EMBEDDING_ENGINE == "":
    log.warning(
        "Local embedding engine is disabled by policy; skipping local embedding model initialization."
    )
    app.state.config.RAG_EMBEDDING_MODEL = ""

if app.state.config.RAG_RERANKING_ENGINE == "":
    log.warning(
        "Local reranking engine is disabled by policy; skipping local reranking model initialization."
    )
...
async def chat_completion(
    ...
    try:
        ...
        # Preserve frontend-sent RAG params
        frontend_rag_params = form_data.get("params", {}).get("rag", {})
        frontend_rag_mode = normalize_rag_mode_value(
            form_data.get("params", {}).get("rag_mode")
        )
        frontend_agent_rag_config = form_data.get("params", {}).get("agent_rag_config", {})
        ...
        metadata = {
            ...
            "params": {
                ...
                "rag": frontend_rag_params,
                "rag_mode": frontend_rag_mode,
                "agent_rag_config": frontend_agent_rag_config,
            },
        }
            ...
            **(
                {
                    ...
                    "rag_reranking_engine": app.state.config.RAG_RERANKING_ENGINE,
                    "rag_reranking_preset_models": app.state.config.RAG_RERANKING_PRESET_MODELS,
