...
from open_webui.utils.rag import normalize_rag_mode
...
def get_builtin_tools(
    ...
    # Knowledge base tools - conditional injection based on the active knowledge boundary
    # for the current request. In Agent mode, chat-attached knowledge should take precedence
    # over folder/model knowledge so prompt/UI/runtime share the same boundary.
    metadata = extra_params.get("__metadata__", {}) or {}
    rag_mode = normalize_rag_mode(metadata)
    agent_rag_config = extra_params.get("__agent_rag_config__") or {}
    agent_scope = agent_rag_config.get("scope", "chat")
    agent_allow_view_knowledge_file = bool(
        agent_rag_config.get("allow_view_knowledge_file", False)
    )
    agent_allow_view_file = bool(agent_rag_config.get("allow_view_file", False))
    agent_allow_view_note = bool(agent_rag_config.get("allow_view_note", False))
    agent_allow_workspace_notes = bool(
        agent_rag_config.get("allow_workspace_notes", False)
    )
    agent_allow_chat_history = bool(
        agent_rag_config.get("allow_chat_history", False)
    )

    chat_knowledge = list(extra_params.get("__model_knowledge__") or [])
    folder_knowledge = extra_params.get("__metadata__", {}).get("folder_knowledge")
    folder_knowledge = list(folder_knowledge or [])
    model_attached_knowledge = list(
        model.get("info", {}).get("meta", {}).get("knowledge", []) or []
    )

    if rag_mode == "agent":
        if chat_knowledge:
            model_knowledge = chat_knowledge
        elif folder_knowledge:
            model_knowledge = folder_knowledge
        else:
            model_knowledge = model_attached_knowledge
    else:
        model_knowledge = model_attached_knowledge
        if folder_knowledge:
            model_knowledge = list(model_knowledge or []) + folder_knowledge
        if rag_mode in {"traditional", "disabled"} and chat_knowledge:
    ...
    if is_builtin_tool_enabled("knowledge"):
        ...
        if rag_mode == "agent":
            ...
            if agent_scope == "global":
                ...
                if agent_allow_view_knowledge_file:
                    builtin_functions.append(view_knowledge_file)
                if agent_allow_view_file:
                    builtin_functions.append(view_file)
                if agent_allow_view_note:
                    builtin_functions.append(view_note)
            else:
                if "collection" in knowledge_types and agent_allow_view_knowledge_file:
                    builtin_functions.append(view_knowledge_file)
                if "file" in knowledge_types and agent_allow_view_file:
                    builtin_functions.append(view_file)
                if "note" in knowledge_types and agent_allow_view_note:
                    builtin_functions.append(view_note)
        elif rag_mode in {"traditional", "disabled"}:
            if "file" in knowledge_types and agent_allow_view_file:
                builtin_functions.append(view_file)
            if "note" in knowledge_types and agent_allow_view_note:
                builtin_functions.append(view_note)
    ...
    if is_builtin_tool_enabled("chats") and agent_allow_chat_history:
    ...
    if (
        ...
        and agent_allow_workspace_notes
    ):
        builtin_functions.extend(
    ...
    for func in builtin_functions:
        callable = get_async_tool_function_and_apply_extra_params(
            func,
            {
                ...
                "__agent_rag_config__": agent_rag_config,
            },
        )
