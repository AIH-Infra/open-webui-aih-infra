...
async def create_new_knowledge(
    ...
    try:
        ...
        log.exception(f"Error creating knowledge base: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
# Response models for chunks API
...
class ChunkStatistics(BaseModel):
    ...
    avg_tokens: float
    avg_chars: float
    token_distribution: dict  # {"0-500": count, "500-1000": count, ...}
...
async def get_knowledge_chunks_by_id(
    id: str,
    limit: Optional[int] = Query(100, description="Maximum number of chunks to return"),
    offset: Optional[int] = Query(0, description="Number of chunks to skip"),
    user=Depends(get_verified_user),
    db: Session = Depends(get_session),
):
    """
    Get chunks from a knowledge base with statistics.
    Returns chunk content, metadata, and distribution statistics.
    """
    ...
    try:
        ...
        result = VECTOR_DB_CLIENT.query(
            ...
            filter=None,
            limit=None,  # Get all chunks for statistics
        )

        if result is None or len(result.ids[0]) == 0:
        ...
        token_distribution = {
            "0-500": sum(1 for t in token_counts if 0 <= t < 500),
        ...
        char_distribution = {
            "0-2000": sum(1 for c in char_counts if 0 <= c < 2000),
        ...
        avg_chars = total_chars / total_count if total_count > 0 else 0.0
        ...
        log.exception(f"Error getting chunks for knowledge {id}: {e}")
        raise HTTPException(
