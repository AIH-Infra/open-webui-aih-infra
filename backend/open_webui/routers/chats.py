...
class ChatMessageDetailResponse(BaseModel):
    ...
    statusHistory: Optional[list] = None
    error: Optional[object] = None
    usage: Optional[dict] = None
    timestamp: Optional[int] = None
    done: bool = True
    childrenIds: list[str] = []
    ragSnapshot: Optional[dict] = None


@router.get(
    "/{id}/messages/{message_id}/detail",
    response_model=Optional[ChatMessageDetailResponse],
)
async def get_chat_message_detail_by_id(
    id: str,
    message_id: str,
    user=Depends(get_verified_user),
    db: Session = Depends(get_session),
    ...
    detail = Chats.get_message_detail_by_id_and_message_id(id, message_id, db=db)
    if not detail:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=ERROR_MESSAGES.NOT_FOUND,
    ...
    return ChatMessageDetailResponse(**detail)
