...
Analyze the chat history to generate {{QUERY_COUNT}} precise, contextually appropriate search queries that reflect the current depth and focus of the discussion. Generate search queries unless it is absolutely certain that no additional information is required. The aim is to retrieve specific, academically rigorous information that addresses the knowledge gaps in the current discussion.

### Guidelines:
- Respond **EXCLUSIVELY** with a JSON object. Any form of extra commentary, explanation, or additional text is strictly prohibited.
- When generating search queries, respond in the format: { "queries": ["query1", "query2"] }, ensuring each query is distinct, focused, and relevant to the current discussion context.
- If and only if it is entirely certain that no useful results can be retrieved by a search, return: { "queries": [] }.
- **Precision over breadth**: Generate queries that use accurate academic terminology and concepts rather than broad, general terms.
- **Multi-language strategy**: For academic discussions involving specific thinkers, theories, or cultural contexts, consider generating queries in multiple relevant languages:
  - Example: For Kant → Chinese academic research + English secondary literature + German primary sources
  - Example: For Foucault → Chinese + English + French resources
  - Example: For Chinese classical thought → Chinese sources + English sinological research
  - Use accurate academic terminology in each language.
- **Track conceptual development**: Pay special attention to how key concepts, theoretical frameworks, and research directions have been clarified and deepened through the dialogue.
- **Utilize assistant's contributions**: Consider both the user's questions AND the assistant's analytical frameworks, suggestions, and identified research directions when formulating queries.
- **Distinguish academic needs**: Identify whether the current discussion requires:
  - Theoretical literature (specific theories, schools of thought, scholarly debates)
  - Case materials (historical events, primary sources, archival materials, specific examples)
  - Background knowledge (historical context, institutional background, cultural environment)
  - Methodological resources (research methods, analytical frameworks, academic paradigms)
- Today's date is: {{CURRENT_DATE}}.
- Generate queries that maximize the relevance and academic rigor for the specific research context.
...
RAG_RERANKING_PRESET_MODELS = PersistentConfig(
    "RAG_RERANKING_PRESET_MODELS",
    "rag.reranking_preset_models",
    [
        model.strip()
        for model in os.environ.get("RAG_RERANKING_PRESET_MODELS", "").split(",")
        if model.strip()
    ],
)
